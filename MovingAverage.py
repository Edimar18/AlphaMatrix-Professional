...
class MVS():
    def calculate_sma(data: pd.DataFrame, period: int) -> pd.Series:
        ...
        """
        return data['Close'].rolling(window=period).mean().dropna()
    def calculate_ema(data: pd.DataFrame, period: int) -> pd.Series:
        ...
        """
        return data['Close'].ewm(span=period, adjust=False).mean().dropna()
    def calculate_wma(data: pd.DataFrame, period: int) -> pd.Series:
        ...
        return data['Close'].rolling(window=period).apply(lambda x: np.dot(x, weights)).dropna()
